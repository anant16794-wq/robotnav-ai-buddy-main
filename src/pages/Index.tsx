...
const Index = () => {
  ...
  const instruction = useMemo(() => {
    if (!s.route.steps.length) return "Enter a destination to calculate a live route.";
    const next = s.route.steps.find((st) => st.command !== "F") ?? s.route.steps[0];
    if (!next) return "Continue forward.";
    if (next.command === "L") return `In ${next.distance} meters, turn left.`;
    if (next.command === "R") return `In ${next.distance} meters, turn right.`;
    if (next.command === "S") return `Destination reached. Stopping.`;
  ...
  const handleEmergency = () => {
    ...
    toast.error("Emergency stop triggered", { description: "Robot halted immediately." });
  };

  const handleSendEsp32 = (cmd: RobotCommand) => {
    const meta = commandMeta(cmd);
    s.applyCommand({ command: cmd, label: meta.label, subtitle: meta.subtitle, reply: `Manual ${meta.label}.` });
  };
      ...
      <div className="mx-auto grid max-w-[1700px] gap-4 xl:grid-cols-[280px_minmax(0,1fr)_360px]">
        ...
        <main className="flex flex-col gap-4 animate-fade-in">
          <header className="glass-card flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-primary">RobotNav Assist</div>
              <h1 className="mt-1 text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Accessible Navigation Console</h1>
          ...
          <section className="grid gap-4 2xl:grid-cols-[minmax(0,1fr)_320px]">
            <div className="flex flex-col gap-4">
              <RouteOverview route={s.route} usedBrowserLocation={s.usedBrowserLocation} />
              <NavigationMap route={s.route} isLoading={s.routeLoading} error={s.routeError} />
            </div>
            <div className="flex flex-col gap-4">
              ...
              />
              <CurrentCommandCard command={s.currentCommand} />
            </div>
          </section>
        ...
        <aside className="flex flex-col gap-4 animate-fade-in xl:sticky xl:top-4 xl:h-[calc(100vh-2rem)] xl:overflow-y-auto scrollbar-thin xl:pr-1">
          ...
          <FamilyMonitorPanel
            ...
            onReturn={() => {
              ...
              s.applyCommand({ command: "F", label: "RETURN", subtitle: "Heading home", reply: "Returning to start." });
            }}
            onChangeRoute={() => {
...
function RouteOverview({ route, usedBrowserLocation }: {
  ...
  return (
    <section className="glass-card grid gap-3 p-4 sm:grid-cols-3">
      <OverviewItem icon={Navigation} label="Route" value={route.totalDistance ? "Active guidance" : "Awaiting destination"} />
      <OverviewItem icon={MapPin} label="Distance" value={formatDistance(route.totalDistance)} />
      <OverviewItem icon={Radio} label="ETA" value={formatDuration(route.durationSeconds)} helper={usedBrowserLocation ? "Using browser location" : "Using saved origin"} />
    </section>
  );
}
